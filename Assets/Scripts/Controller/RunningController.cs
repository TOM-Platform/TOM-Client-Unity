...
public class RunningController : HomeController
{
    ...
    {
        InvokeRepeating(nameof(RequestRunningLiveData), SOCKET_COMMUNICATION_START_DELAY_SECONDS, RUNNING_DATA_REQUEST_DELAY_SECONDS);
        InvokeRepeating(nameof(RequestDirectionData), SOCKET_COMMUNICATION_START_DELAY_SECONDS, DIRECTION_DATA_REQUEST_DELAY_SECONDS);
    ...
    private bool ProcessDataBytes(byte[] byteData)
    {
        try
        {
            ...
            if (dataType == DataTypes.RUNNING_LIVE_DATA || dataType == DataTypes.RUNNING_LIVE_UNIT || dataType == DataTypes.RUNNING_LIVE_ALERT)
            {
                try
                {
            ...
            else if (dataType == DataTypes.RUNNNING_TYPE_POSITION_MAPPING_DATA)
            {
                try
                {
                    RunningTypePositionMappingData runningTypePositionMapping = RunningTypePositionMappingData.Parser.ParseFrom(data);
                ...
                {
                    Debug.LogError("Bytes received cannot be decoded as RunningTypePositionMappingData: " + e.Message);
                    return false;
                }
    ...
    private void SpeakInstructions()
    {
        ...
        if (!(speechList.Count == 0 || textToSpeech.IsSpeaking()))
        {
            ...
            var directionInstr = speechList.FirstOrDefault(instruction => instruction.Item1 == DataTypes.DIRECTION_DATA);
            ...
            else
            {
                ...
                var runningInstr = speechList.FirstOrDefault(instruction => instruction.Item1 == DataTypes.RUNNING_LIVE_DATA);
    ...
    public void SelectSpeedTraining()
    {
        ...
        SendRequestToServer(DataTypes.REQUEST_RUNNING_TRAINING_MODE_DATA, RunningTrainingMode.SpeedTraining.ToString());
    ...
    public void SelectDistanceTraining()
    {
        ...
        SendRequestToServer(DataTypes.REQUEST_RUNNING_TRAINING_MODE_DATA, RunningTrainingMode.DistanceTraining.ToString());
    ...
    {
        InvokeRepeating(nameof(RunningController.SendRunningCameraDataToServer), SOCKET_COMMUNICATION_START_DELAY_SECONDS, SEND_CAMERA_DATA_DELAY_SECONDS);
    }
