...
public class ImageLoader : MonoBehaviour, IMixedRealityPointerHandler
{
    ...
    private void LoadImageContent(GameObject imageButton, byte[] routeMapImage, string imageName, RouteData routeData)
    {
    ...
    public void ImageButtonClickHandler(string imageLabel)
    {
        ...
        Dialog confirmationDialog = Dialog.Open(confirmationDialogPrefab, DialogButtonType.Yes | DialogButtonType.No, "Confirm Route", $"Are you sure you want to choose {imageLabel}?", true);
    ...
    public void OnClosedDialogEvent(DialogResult result)
    {
        if (result.Result == DialogButtonType.Yes)
        {
            ...
            if (selectedImageIndex > 0 && selectedImageIndex <= routesCount)
            {
                ...
                runningController.SendRequestToServer(DataTypes.REQUEST_CHOSEN_ROUTE_DATA, selectedImageIndex.ToString());
            ...
            else
            {
                ...
                runningUIController.UpdateInstructions(DataTypes.DIRECTION_DATA, "Unknown route selected, please try again");
            }
        }
        else
        {
