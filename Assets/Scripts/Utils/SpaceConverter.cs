    ...
    public class SpaceConverter
    {
        ...
        public static Frame ApplyBoundingBox(Transform obj, Camera camera, Resolution resolution, bool enableLog)
        {
            ...
            int croppedImageWidth =
                (int)(Math.Abs(set.bottomLeft.x - set.topRight.x) + Math.Abs(set.topLeft.x - set.bottomRight.x)) / 2;
            int CroppedImageHeight =
                (int)(Math.Abs(set.bottomLeft.y - set.topRight.y) + Math.Abs(set.topLeft.y - set.bottomRight.y)) / 2;
            ...
            Frame result = new(startPositionX, startPositionY, croppedImageWidth, CroppedImageHeight,
        ...
        public static CornerSet2D GetCornersOnScreen(Transform obj, Camera camera, bool enableLog)
        {
            ...
            Vector3 topLeft_World = GetWorldCoordinates(centerPosition_World, rotation_World, CornerPosition.TOP_LEFT,
                width, height);
            Vector3 topRight_World = GetWorldCoordinates(centerPosition_World, rotation_World, CornerPosition.TOP_RIGHT,
                width, height);
            Vector3 bottomLeft_World = GetWorldCoordinates(centerPosition_World, rotation_World,
                CornerPosition.BOTTOM_LEFT, width, height);
            Vector3 bottomRight_World = GetWorldCoordinates(centerPosition_World, rotation_World,
                CornerPosition.BOTTOM_RIGHT, width, height);
        ...
        public static Vector3 GetWorldCoordinates(Vector3 centerPosition, Quaternion centerRotation,
            CornerPosition type, float width, float height)
        {
        ...
        {
            return new Vector2(position.x / resolution.width, position.y / resolution.height);
        }
        ...
        public static Vector2 CroppedImageToScreen(Vector2 position, Resolution resolution, Frame cropParameters)
        {
            ...
            return new Vector2(position_Image.x / resolution.width, position_Image.y / resolution.height);
        }

        // position = the screen position, normalized
        public static Vector3 ScreenToCamera(Vector2 position, Vector2 viewportProjectionSize, Vector2 offset,
            float prevSqrDistance)
        {
            float x = position.x;
            float y = position.y;
        ...
        public static Vector3 ScreenToCamera(Vector2 position, Camera camera, Vector3 prevCameraPosition,
            Vector3 prevBoundingBoxPosition)
        {
            ...
            // Converts the corners to the display screen space, and then normalizes them
            Vector2 offset = new(X_OFFSET_NORMALIZED * viewportProjectionWidth,
                Y_OFFSET_NORMALIZED * viewportProjectionHeight);
        ...
        public static void GetViewportProjectionSizeAndOffset(Vector3 position, Camera camera,
            out Vector2 viewportProjectionSize, out Vector2 offset)
        {
            // Gets the FOVs
            float aspectRatio = camera.aspect;
            float VFOV = (float)(camera.fieldOfView * Math.PI / 180);
            ...
            // Converts the corners to the display screen space, and then normalizes them
            offset = new Vector2(X_OFFSET_NORMALIZED * viewportProjectionWidth, Y_OFFSET_NORMALIZED * viewportProjectionHeight);
            viewportProjectionWidth *= xProportion;
            viewportProjectionHeight *= yProportion;
            viewportProjectionSize = new Vector2(viewportProjectionWidth, viewportProjectionHeight);
        }
